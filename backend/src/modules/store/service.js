...
const createStore = async (ownerId, storeData) => {
  ...
  if (existingStore) {
    const error = new Error('You already have a store or a pending store application');
...
const updateStore = async (ownerId, storeId, storeData) => {
  ...
  if (storeData.description !== undefined) store.description = storeData.description;
