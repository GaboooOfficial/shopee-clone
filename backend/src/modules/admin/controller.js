...
const approveStore = async (req, res) => {
  try {
    const store = await adminService.updateStoreStatus(req.params.id, 'approved');
...
const rejectStore = async (req, res) => {
  try {
    const store = await adminService.updateStoreStatus(req.params.id, 'rejected');
...
const toggleStoreActive = async (req, res) => {
  try {
    ...
    return sendSuccess(res, store, store.isDeactivated ? 'Store deactivated' : 'Store activated');
  } catch (error) {
    return sendError(res, error.message, error.statusCode || 500);
  }
};
...
const toggleUserActive = async (req, res) => {
  try {
    ...
    return sendSuccess(res, user, user.status === 'deactivated' ? 'Account deactivated' : 'Account activated');
  } catch (error) {
    return sendError(res, error.message, error.statusCode || 500);
  }
};
