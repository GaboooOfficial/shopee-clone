  ...
  try {
    ...
    if (wss) {
      wss.clients.forEach((client) => {
        if (client.userId === receiverId.toString() && client.readyState === 1) { // 1 is WebSocket.OPEN
...
const getConversation = async (req, res) => {
  try {
    ...
    const messages = await messageService.getConversation(req.user._id, otherUserId);
...
const getConversationsList = async (req, res) => {
  try {
    const conversations = await messageService.getConversationsList(req.user._id);
