...
export class CourierComponent implements OnInit {
  ...
  calculateStats() {
    ...
    this.stats.transitJobs = this.pendingDeliveries.filter(
      (d) => d.status === 'intransit',
    ).length;
    this.stats.deliveredJobs = this.pendingDeliveries.filter(
      (d) => d.status === 'package delivered',
    ).length;
    this.stats.failedJobs = this.pendingDeliveries.filter(
      (d) => d.status === 'unsuccessful',
    ).length;
  }
  ...
  updateDeliveryStatus() {
    ...
    this.http
      .patch<any>(
        `${this.baseUrl}/orders/${this.selectedOrderIdForUpdate}/status`,
        payload,
      ...
      .subscribe({
        ...
        error: (err) => {
          this.errorMessage =
            err.error?.message || 'Failed to update delivery status';
          this.clearMessages();
        },
      });
  }
